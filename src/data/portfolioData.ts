...
export const personalInfo = {
  ...
  heroSummary:
    "Full-stack web engineer with 3+ years of experience in API design, performance monitoring, and code quality across PHP/Laravel and React/Node stacks.",
  credly: credlyProfileUrl,
};

export const fitKeywords = [
...
export const experiences: Experience[] = cvExperiences.map(
  (item: (typeof cvExperiences)[number]) => ({
...
export const skills: string[] = (
  Array.from(
    new Set(
      Object.values(cvSkills as Record<string, string[]>).flatMap(
        (skillGroup: string[]) => skillGroup,
      ),
    ),
  ) as string[]
).slice(0, 32);
...
export const certificates: Certificate[] = [
  ...
  ...cvCertificates.map(
    (item: (typeof cvCertificates)[number], idx: number) => ({
      title: item.title,
      ...
      color:
        certificateColors[
          (idx + credlyBadges.length) % certificateColors.length
        ],
    }),
  ),
];
